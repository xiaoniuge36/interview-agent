...
export function AdminOverview(props: AdminOverviewProps) {
  ...
  const pending = dashboard?.stats.pendingCandidates ?? 0;
  return (
...
function GovernanceCommand(props: {
  ...
  return (
    <Card className="admin-dense-card" title="今日治理待办">
      <Row gutter={[COMPACT_GUTTER, COMPACT_GUTTER]}>
        <Col sm={8} xs={24}>
          <Statistic title="待审核候选题" value={props.pending} />
        </Col>
        <Col sm={8} xs={24}>
          <Statistic
            suffix="%"
        ...
        <Col sm={8} xs={24}>
          <Statistic title="失败导入" value={failedImports(props.dashboard)} />
        </Col>
      </Row>
      <div className="admin-quick-actions">
        <Button icon={<CloudUploadOutlined />} onClick={() => props.onNavigate('imports')}>
          导入资料
        </Button>
        <Button
        ...
        <Button icon={<RadarChartOutlined />} onClick={() => props.onNavigate('runtime')}>
          运行观测
        </Button>
      </div>
    </Card>
...
function RecentRuns({
  ...
  const columns: TableProps<Dashboard['recentRuns'][number]>['columns'] = [
    { title: '阶段', dataIndex: 'stage', ellipsis: true },
    {
      ...
      dataIndex: 'status',
      width: 96,
      render: (status) => <Tag color={runColor(status)}>{runStatus(status)}</Tag>,
    },
    {
      title: '耗时',
      dataIndex: 'latencyMs',
  ...
  return (
    <Card
      ...
      extra={
        <Button type="link" onClick={() => onNavigate('runtime')}>
          全部运行
        </Button>
      }
      title="最近 Agent 运行"
    >
      {dashboard.recentRuns.length ? (
        <Table
          columns={columns}
          dataSource={dashboard.recentRuns}
          pagination={false}
          rowKey="id"
          size="small"
        />
      ) : (
        <Typography.Text type="secondary">暂无 Agent 运行记录。</Typography.Text>
      )}
    </Card>
  );
}
  ...
  return (
    <Card className="admin-dense-card" title="管理模块">
      <div className="admin-module-actions">
        {items.map((item) => (
          <Button icon={item.icon} key={item.view} onClick={() => onNavigate(item.view)}>
            {item.label}
          </Button>
        ))}
      </div>
...
function runColor(status: Dashboard['recentRuns'][number]['status']): string {
  return { running: 'processing', succeeded: 'success', failed: 'error', fallback: 'warning' }[
    status
  ];
}

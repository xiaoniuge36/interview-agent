...
export const DATA_AI_ROLE_TEMPLATES: RoleTemplate[] = [
  defineRole({
    ...
    title: '数据分析师',
    jdText:
      '围绕业务目标完成指标体系、专题分析和实验评估，把分散数据转化为可执行的增长与经营建议。',
    companyContext: '业务需要更快识别增长瓶颈，团队希望通过统一口径与高质量分析支持关键决策。',
    communicationText:
      '从问题定义、数据口径、分析路径到建议落地讲完整，说明结论如何被验证并产生结果。',
  ...
  defineRole({
    ...
    communicationText:
      '说明你如何设计分层模型、治理数据质量、定位链路问题，并服务分析或算法下游需求。',
  ...
  defineRole({
    ...
    title: '算法工程师',
    jdText:
      '负责推荐、排序、预测或搜索等算法能力建设，完成数据特征、模型训练、线上实验和效果迭代闭环。',
    companyContext:
      '业务希望通过智能决策提升转化与效率，团队需要兼顾离线指标、在线效果和工程可落地性。',
    communicationText:
      '用完整案例说明目标函数、特征或模型选择、实验结果及失败复盘，避免只讲模型结构。',
  ...
  defineRole({
    ...
    title: '机器学习工程师',
    jdText:
      '负责机器学习模型的训练、部署、监控和持续迭代，推动模型能力从实验环境稳定进入线上业务。',
    companyContext:
      '团队已有多个模型原型，当前挑战是提高线上可靠性、数据闭环效率和跨团队协作效率。',
  ...
  defineRole({
    ...
    title: 'AI Agent 工程师',
    jdText:
      '负责 AI Agent、RAG 与工具调用链路设计，把大模型能力组合为可靠、可评估、可运营的业务产品。',
    companyContext:
      '团队正从 Demo 走向生产应用，需要处理模型效果、成本、安全、稳定性与人工协同等问题。',
    communicationText:
      '描述提示词、检索、工具编排和评测体系如何协同，明确失败兜底和对业务目标的贡献。',

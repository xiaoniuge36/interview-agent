...
export const GROWTH_OPERATIONS_ROLE_TEMPLATES: RoleTemplate[] = [
  defineRole({
    ...
    title: '增长运营',
    jdText:
      '负责增长策略、用户分层和活动机制运营，围绕拉新、激活、留存与转化建立可复制的增长闭环。',
    companyContext:
      '增长从粗放投放转向精细运营，团队需要更好地理解用户路径并提高每一环的转化效率。',
    communicationText:
      '结合完整案例说明目标拆解、策略选择、实验节奏和最终指标提升，体现因果判断能力。',
  ...
  defineRole({
    ...
    title: '内容运营',
    jdText:
      '负责内容策略、生产机制和分发运营，通过高质量内容建立用户认知、提升活跃并服务业务转化。',
  ...
  defineRole({
    ...
    companyContext:
      '用户规模增长后，团队需要从统一触达转向分群运营，并建立可持续的用户价值运营体系。',
    communicationText:
      '重点讲用户分层、触发机制、策略效果和优化过程，用数据说明对留存或转化的影响。',
  ...
  defineRole({
    ...
    companyContext:
      '社区进入规模化阶段，团队既要激励优质内容，也要控制低质内容和冲突带来的体验风险。',
    communicationText:
      '说明生态问题如何被识别、规则与活动如何设计，以及如何平衡增长、秩序和用户感受。',
  ...
  defineRole({
    ...
    title: '投放运营',
    jdText:
      '负责广告投放策略、渠道优化和归因分析，通过预算配置、素材迭代和实验提升获客效率与 ROI。',
    companyContext:
      '获客成本持续上升，团队需要识别高价值渠道与用户，并让投放结果与后续转化价值相连接。',
  ...
  defineRole({
    ...
    communicationText:
      '结合大促或日常经营案例说明目标拆解、策略组合、关键数据和异常问题的处理过程。',

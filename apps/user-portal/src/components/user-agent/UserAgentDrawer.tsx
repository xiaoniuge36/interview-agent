...
export function UserAgentDrawer(props: Props) {
  ...
  return (
    <div className="user-agent-layer" data-page-agent-not-interactive="true">
      <button
        aria-label="关闭 AI 刷题教练"
        className="user-agent-backdrop"
        type="button"
...
function DrawerHeader(props: {
  ...
  return (
    <header className="user-agent-drawer-header">
      <span className="user-agent-header-mark" aria-hidden="true">
        ✦
      </span>
      <div>
      ...
      <button className="user-agent-header-action" type="button" onClick={props.onSetup}>
        模型
      </button>
      <button aria-label="关闭" className="user-agent-close" type="button" onClick={props.onClose}>
        ×
      </button>
    </header>
  );
    ...
    <main className="user-agent-chat-pane">
      <div className={`user-agent-status${props.status === 'running' ? ' is-running' : ''}`}>
        ...
        <p>
          ...
          <small>涉及模型消耗或提交操作时，我会先请你确认。</small>
        </p>
      </div>
      <ChatNotices {...props} />
      <ChatContent {...props} />
  ...
  return (
    <>
      {props.pendingQuestion ? (
        <Confirmation question={props.pendingQuestion} onAnswer={props.onAnswer} />
      ) : null}
...
function ChatContent(props: Props) {
  ...
  return (
    <Conversation
      ...
      executionSteps={props.executionSteps}
...
function Confirmation(props: { question: string; onAnswer: (answer: string) => void }) {
  return (
    <div className="user-agent-confirmation">
      ...
      <div>
        <button type="button" onClick={() => props.onAnswer('用户确认继续。')}>
          确认继续
        </button>
        <button type="button" onClick={() => props.onAnswer('用户取消本次操作。')}>
          取消
        </button>
      </div>
    </div>
...
function Conversation(props: {
  ...
  return (
    <>
      <div className="user-agent-chat-heading">
        <strong>{props.currentTitle}</strong>
        <span>当前训练对话</span>
      </div>
      <ExecutionTrace steps={props.executionSteps} />
      {props.messages.length ? (
        <MessageList messages={props.messages} />
      ) : (
        <QuickActions busy={props.busy} context={props.pageContext} onSend={props.onSend} />
      )}
      <div className="user-agent-metrics">
        <span>{props.activity}</span>
        <span>本轮约 {props.tokens.toLocaleString()} tokens</span>
      </div>
...
function ExecutionTrace({ steps }: { steps: PageAgentExecutionStep[] }) {
  if (!steps.length) return null;
  return (
    <section aria-label="AI 执行过程" className="user-agent-execution-trace">
      <div>
        <strong>执行过程</strong>
        <span>实时进度，不会代替你操作</span>
      </div>
      <ol>
  ...
  return (
    <section className="user-agent-quick-actions" aria-label="常用训练建议">
      <div>
        ...
        <span className="user-agent-eyebrow">当前场景</span>
      </div>
      <div className="user-agent-quick-grid">
        {props.context.quickActions.map((action) => (
          <button
            disabled={props.busy}
            key={action.id}
            type="button"

...
AI 刷题教练接入时同步收紧练习播放器的交互节奏：

- 当前答案有未保存修改时，切题前明确确认，避免误以为已经同步到服务端。
- 当前题保存成功后提供“保存并下一题”，减少保存和翻页之间的重复操作。
- 单题 AI 评价在调用用户模型前提示会产生模型消耗。
- 题目导航在保存、解析加载和模型评价期间锁定，避免异步结果落到用户当前正在查看的另一题上。
- 答题区显示字数和保存状态；解析区区分标准答案、真实模型评价和错误提示。
- 桌面端保持题目导航、主答题区、教练面板三栏结构，压缩无效留白；移动端按题目导航、答题、反馈顺序展开。

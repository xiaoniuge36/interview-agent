...
- [ ] Step 1: 写 Pydantic 与 Zod schema 的跨端 contract 测试。
...
- [ ] Step 2: 增加 operation practice_report 和受限 grant。
...
- [ ] Step 3: Product API 仅传本次 session、evaluations、已授权 retrieval context 和 traceId。

  const grant = this.grants.issue({ sessionId, commandId, traceId, operation: "practice_report" });

- [ ] Step 4: 运行 contract 与 gateway 测试。
...
- [ ] Step 1: 为 valid、一次 repair 成功、第二次失败 fallback、checkpoint 恢复写 Pytest。
...
- [ ] Step 3: fallback 只使用已验证 evaluation，不调用模型。

  return deterministic_report(request.session, request.evaluations, request.trace_id)

- [ ] Step 4: 注册 Runtime endpoint 和 checkpointer。
...
- [ ] Step 1: 创建固定评分和报告 cases，包含事实、分数区间、缺失点、schema 和 evidence。

  {"id":"weak-system-design","answer":"只描述组件", "expectedScoreRange":[35,60], "requiredMissingPoints":["容量规划"]}

- [ ] Step 2: runner 先执行确定性断言，再选择性调用 Judge。
...
- [ ] Step 3: 将 schema pass、fallback rate、Golden 成绩写 JSON artifact。
...
- [ ] Step 1: 运行 Python lint/typecheck/test、contracts、agent eval 与定向 E2E。

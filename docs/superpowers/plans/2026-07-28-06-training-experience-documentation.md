...
- [x] Step 1: 写分页、下架题目历史、跨租户和已复练状态测试。
...
- [x] Step 2: 使用低分 EvaluationResult 加 MemoryEvent evidence 查询，返回历史快照而非题库可编辑对象。

  return MistakeBookSchema.parse({ questionSnapshot, score, evidence, canStartReview: question.status === "published" });

- [x] Step 3: 复练只能创建引用已发布题目的 weakness_review session。
...
- [x] Step 1: 写错题空态、复练、推荐 evidence、未完成训练优先级测试。
...
- [x] Step 2: 将首页、报告、面试复练都跳转到同一 session creation API。

  await startWeaknessReview({ itemIds, loadRecommendations, createSession, router });

- [x] Step 3: 展示证据来源、题目数量和预计时间，始终保留自主选题。
...
- [x] Step 1: 为 job、embedding 覆盖率、retrieval latency、schema pass、fallback、预算拒绝计数写聚合测试。

  expect(metrics).toMatchObject({ deadLetterJobs: 1, retrievalCoverage: 0.8, fallbackRate: 0.1 });

- [x] Step 2: 复用 PlatformAiAnalytics 查询层，增加脱敏聚合，不返回 prompt、答案或 key。

  return { retrievalLatencyMs, schemaPassRate, fallbackRate, budgetRejected, deadLetterJobs };

- [x] Step 3: 运行 Admin 测试和 build。
...
- [x] Step 1: 扩展 E2E 为资产发布、embedding 完成、训练、记忆、推荐、面试复练闭环。

  await expect(page.getByLabel("推荐依据")).toBeVisible();
  await expect(page.getByRole("button", { name: "复练薄弱项" })).toBeVisible();

- [x] Step 2: README 能力矩阵只标记已有自动化证据的能力。

  | Memory 闭环 | 已交付 | practice/interview integration + user-practice E2E |

- [x] Step 3: 创建 DELIVERED 索引，将历史 plan/spec 标记为 delivered、superseded 或 active。

Run: pnpm test:e2e && pnpm verify

Expected: 隔离环境的完整剧本和全仓库门禁通过。

Result (2026-07-29): Contracts 55/55、Product API 332 passed / 16 skipped、Portal 363/363、Admin 串行重跑 152/152、Runtime 43 passed / 1 skipped；根 `typecheck` 8/8 tasks、根 `build` 6/6 tasks；正式 `user-practice.spec.ts` 退出码 0，4 passed / 0 failed。根 `format:check` 的 83 个历史文件和根 `lint` 的历史/客观题独立补丁错误按既有基线记录，Plan 05/06 定向门禁通过。

### Task 5: 最终审阅和提交

- [x] Step 1: 检查文档声明、E2E 名称和实际实现一致。

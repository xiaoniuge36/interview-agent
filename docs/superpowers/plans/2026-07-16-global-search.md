...
- [ ] **Step 4: 浏览器关键流**（桌面关键流已通过；已登录 Chrome 标签不接受临时 375px 视口覆盖，移动端仅完成 CSS 断点审查）
...
- Create: `apps/user-portal/src/components/search/GlobalSearchTrigger.tsx`
- Create: `apps/user-portal/src/components/search/GlobalSearchTrigger.test.tsx`
- Create: `apps/user-portal/src/app/styles/global-search-dock.css`
- Modify: `apps/user-portal/src/components/UserShell.tsx`
- Modify: `apps/user-portal/src/app/globals.css`

- [x] **Step 1: 先写公共入口语义测试并确认红灯**
- [x] **Step 2: 实现顶部触发器并复用现有 Provider**
- [x] **Step 3: 接入 UserShell，完成桌面与窄屏样式**
- [ ] **Step 4: 浏览器验证**（当前已登录页面停留在 Auth Loading，服务返回 200；不重启用户现有服务）

...
```mermaid
flowchart LR
  Web["apps/user-portal\nNext.js User App"] -- OpenAPI --> API["apps/product-api\nNestJS Product API"]
...
```mermaid
flowchart TD
  UserApp["apps/user-portal / 用户端 Next.js"] --> API["apps/product-api / NestJS Product API"]
  Admin["apps/admin-console / 后台端 Next.js"] --> API
...
裁决规则：业务规则只进入 `apps/product-api` 和 `packages/authz`；Agent 规则只进入 `apps/agent-runtime`；跨语言字段和枚举只进入 `packages/contracts`，不能在前端、后端、Agent 三处各自手写一份。
...
执行口径：`infra/docker/docker-compose.yml` 负责拉起基础设施；`apps/product-api`、`apps/agent-runtime`、`apps/user-portal` 可以先本机运行，通过 `.env.local` 连接容器服务。等核心链路稳定后，再决定是否把应用服务也纳入 Compose profile。第一阶段不要把 Docker 编排做成复杂微服务发布系统，它只承担“本地一键启动底座”的职责。
...
| 服务 | 本地 | 线上建议 |
|---|---|---|
| `apps/user-portal` | Next.js dev server | 独立 Web 服务或 Vercel/Node runtime。 |
| `apps/admin-console` | Next.js dev server | 独立后台 Web 服务，后台域名单独鉴权。 |
| `apps/product-api` | NestJS dev server | 独立 API 服务，内网访问数据库、Redis、MinIO。 |
...
TypeScript 产品层分成 `apps/user-portal`、`apps/admin-console`、`apps/product-api` 三个应用。它们共享契约、UI token、权限客户端和观测规范，但不共享业务页面状态。

### 应用拆分

| 应用 | 技术 | 负责 | 不负责 |
|---|---|---|---|
| `apps/user-portal` | Next.js App Router + RSC + Tailwind v4 + shadcn/ui | 用户端训练、刷题、模拟面试、报告阅读、画像查看。 | 不维护题库审核，不配置模型，不直接调用 Agent。 |
| `apps/admin-console` | Next.js App Router + TanStack Table + shadcn/ui | 题库治理、导入审核、Prompt/模型配置、审计查看。 | 不读取用户私有训练明细，不代替用户训练。 |
| `apps/product-api` | NestJS + Prisma + OpenAPI + BullMQ | 业务事实源、权限、状态机、审计、Agent 调用代理。 | 不写复杂 prompt，不做多轮 Agent 推理。 |
...
前端权限只做“体验层显隐”，不做最终裁决。任何敏感动作必须由 `apps/product-api` 重新鉴权、校验状态机并写审计。
...
| 模块 | 最终建议 |
|---|---|
| 主体架构 | 前后端分离 + Product API / Agent Runtime 分离：`apps/user-portal`、`apps/admin-console`、`apps/product-api`、`apps/agent-runtime`。 |
